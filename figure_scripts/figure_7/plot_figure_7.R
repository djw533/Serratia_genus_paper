...
#get the names of the gff files extracted:
...
# 
# svg("synteny.svg",width = 15, height = 130)
...
## get examples for each prodigiosin gene cluster in each

##take examples for the different phylogroups - used this to set on the specific selected examples:
...
# serratia_metadata <- read.csv("~/Documents/thesis/chapter_1/5_All_serratia_tr/march_2020_tree_metadata_2.csv", header = T,
#                               quote = "",
#                               stringsAsFactors = F,
#                               comment.char = "") %>%
#   select(File_prefix, X..contigs) %>%
#   rename(strain = File_prefix, num_contigs = X..contigs) 
# 
# 
# 
# pig_present_genomes_w_contigs <- pig_present_genomes %>%
#   left_join(serratia_metadata)

#### 3 - clean up ####


#### 13 - clean up ####
for (filename in list.files(path = "../../figshare_data/prodigiosin/1_extracted_gffs",
                            full.names = TRUE)) {
  file.remove(filename)
}

#remove the directory:
unlink("../../figshare_data/prodigiosin/1_extracted_gffs",
       recursive = TRUE)
