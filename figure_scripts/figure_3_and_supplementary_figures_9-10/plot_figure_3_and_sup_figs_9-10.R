...
#make a dataframe for both fastbaps and the fastANI groups:

fastbaps_and_phylogroups <-  fastani_data %>%
  select(Name, X95) %>% # only take the name of the strains and the clusters when cut off at 95% ANI
  mutate(strain = gsub("#","_",Name)) %>% # remove hashes from WSI lane id's
  select(!Name) %>%
  rename(phylogroup = X95) %>%
...
#### how many genomes are in each lineage?:
...
##for a test - remove all lineages that only have <= 5 genomes:
